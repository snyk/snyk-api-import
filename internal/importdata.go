...
func GenerateImportTargets(ctx context.Context, orgsFile, source string) ([]ImportTarget, error) {
	return GenerateImportTargetsWithLogPath(ctx, orgsFile, source, "", nil)
}

// GenerateImportTargetsWithLogPath generates import targets. An empty repoFilter
// means "all repositories"; otherwise generation is restricted to the named
// repositories (GitHub PAT sources only).
func GenerateImportTargetsWithLogPath(ctx context.Context, orgsFile, source, logPath string, repoFilter []string) ([]ImportTarget, error) {
	...
	if source == "bitbucket-cloud-app" {
		...
		for _, org := range orgs.Orgs {
			...
			// Restrict to an explicit repo list before the per-repo loop, which
			// performs Snyk org and integration lookups for every repository.
			if repoSet := newRepoFilter(repoFilter); repoSet != nil {
				...
				if len(kept) == 0 {
					Logger.Warnf("Repo filter matched no repositories in org %s", orgName)
				}
				repos = kept
			}
