...
func TestE2E_GitHub_FullWorkflow(t *testing.T) {
	...
	// Target the test org directly instead of listing every org on the token
	githubConfig.ExtraOrgsDataArgs = []string{"--githubOrgs=" + cfg.GitHubTestOrg}
	// Optionally limit sync to specific repos. Without this, sync enumerates every
	// repository the token can see in the org, including all public ones.
	githubConfig.ExtraSyncArgs = nil
	githubConfig.ExtraImportDataArgs = nil
	if cfg.GitHubTestRepos != "" {
		githubConfig.ExtraSyncArgs = []string{"--githubRepos=" + cfg.GitHubTestRepos}
		githubConfig.ExtraImportDataArgs = []string{"--githubRepos=" + cfg.GitHubTestRepos}
	}
