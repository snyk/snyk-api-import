...
// newRepoFilter builds a case-insensitive lookup of repository names. A nil
// return means "no filtering" so callers can treat an empty list as unrestricted.
func newRepoFilter(repos []string) map[string]bool {
	if len(repos) == 0 {
		return nil
	}
...
// filterSnykProjectsByRepo restricts Snyk projects to the given repository set.
// This must be applied whenever the discovered source repos are restricted:
// CompareStates treats any Snyk project without a matching source entry as
// stale, so filtering only the source side would report every other project as
// removable (and would deactivate them on a non-dry-run).
func filterSnykProjectsByRepo(projects []map[string]interface{}, repoSet map[string]bool) []map[string]interface{} {
	if repoSet == nil {
		return projects
	}
	...
	for _, p := range projects {
		...
		// Snyk project names look like "owner/repo:manifest" or "owner/repo".
		repoPart := strings.SplitN(name, ":", 2)[0]
		repo := repoPart
		if idx := strings.Index(repoPart, "/"); idx >= 0 {
			repo = repoPart[idx+1:]
		}
		if repoSet[strings.ToLower(repo)] {
...
func DiscoverManifestsOptimized(ctx context.Context, auth GitHubAuth, orgName string, manifestTypes []string, cacheDir string, snykOrgID string, repoFilter []string) ([]map[string]interface{}, error) {
	startTime := time.Now()
	...
	githubClient.UserAgent = buildinfo.UserAgent()
	...
	// Restrict to an explicit repo list before the per-repo branch/tree calls,
	// which are the expensive part of discovery.
	if repoSet := newRepoFilter(repoFilter); repoSet != nil {
		var kept []map[string]string
		for _, repo := range repos {
			if repoSet[strings.ToLower(repo["name"])] {
				kept = append(kept, repo)
			}
		}
		Logger.Infof("Repo filter applied: %d of %d repositories in org %s selected",
			len(kept), len(repos), orgName)
		for name := range repoSet {
			...
			if !found {
				Logger.Warnf("Repo filter entry %q not found in org %s", name, orgName)
			}
		}
		repos = kept
	}
...
func SyncGitHub(orgID, source, orgsFile, targetsFile string, dryRun bool, snykLogPath string, enableBranchUpdateFallback bool, useCache bool, repoFilter []string) error {
	ctx := context.Background()
	return SyncGitHubContext(ctx, orgID, source, orgsFile, targetsFile, dryRun, snykLogPath, enableBranchUpdateFallback, useCache, repoFilter)
}

// SyncGitHubContext performs sync for GitHub with context support
func SyncGitHubContext(ctx context.Context, orgID, source, orgsFile, targetsFile string, dryRun bool, snykLogPath string, enableBranchUpdateFallback bool, useCache bool, repoFilter []string) error {
	startTime := time.Now()
	...
	for org := range orgSet {
		...
		enrichedRepos, err := DiscoverManifestsOptimized(ctx, auth, org, manifestTypes, cacheDir, orgID, repoFilter)
		if err != nil {
	...
	// Keep both sides of the comparison scoped to the same repositories.
	if repoSet := newRepoFilter(repoFilter); repoSet != nil {
		before := len(snykProjects)
		snykProjects = filterSnykProjectsByRepo(snykProjects, repoSet)
		Logger.Infof("Repo filter applied to Snyk projects: %d of %d retained", len(snykProjects), before)
	}
