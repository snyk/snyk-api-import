...
Fine-grained personal access tokens **cannot auto-discover organizations**. GitHub
returns `200 OK` with an empty list for `GET /user/orgs` when the request uses a
fine-grained token, so `orgs:data` finds no organizations and reports none.

Specify the organizations explicitly instead:
...
With an explicit org list, a fine-grained token needs only these **repository**
permissions:

| Permission | Access | Used for                                              |
|------------|--------|-------------------------------------------------------|
| Metadata   | Read   | Listing organization repositories                     |
| Contents   | Read   | Reading default branches and file trees during `sync` |

No organization-level permissions are required. Set the token's resource owner to the
organization you are importing and grant access to the relevant repositories.
