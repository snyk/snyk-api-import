...
- Generating the data to create Organizations in Snyk
  ...
  - [Azure](#azure)
- [Creating the organizations](#options-for-creating-organizations-in-snyk)
  - [via the API](#via-api)
...
```shell
  --source             The source of the targets to be imported
                       ...
                       Bitbucket Server, Bitbucket Cloud, Bitbucket Cloud App)                [required]
...
**Note that the URL for Bitbucket Cloud is [https://bitbucket.org/](https://bitbucket.org/)**
...
1. Once the file is created, you can feed it to the [orgs:create command](#creating-organizations-in-snyk).

## Options for creating Organizations in Snyk

Use the generated data file to help create the organizations via API or use the provided util.

## via API

Use the generated data to feed into Snyk [Orgs API](https://docs.snyk.io/snyk-api/reference/organizations-v1#post-org) to generate the organizations within a group.
...
- have [notifications disabled](https://docs.snyk.io/snyk-api/using-specific-snyk-apis/webhooks-apis/about-webhooks#notifications) for emails etc to avoid receiving import notifications
- have the [fix PRs and PR checks disabled](https://docs.snyk.io/snyk-api/reference/integrations-v1) until import is complete to avoid sending extra requests to SCMs (Github/Gitlab/Bitbucket etc)
