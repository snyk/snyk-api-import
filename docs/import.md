...
- you will need your Snyk API token, with correct scope & [admin access for all Organizations](https://docs.snyk.io/snyk-api/reference/import-projects-v1) you are importing to. **Github Integration Note**: As Github is both an auth & integration, how the integration is done has an effect on usage:
...
- Recommended: have [notifications disabled](https://docs.snyk.io/snyk-api/using-specific-snyk-apis/webhooks-apis/about-webhooks#notifications) for emails etc to avoid receiving import notifications
- Recommended: have the [fix PRs and PR checks disabled](https://docs.snyk.io/snyk-api/reference/integrations-v1) until import is complete to avoid sending extra requests to SCMs (Github/Gitlab/Bitbucket etc)
...
- `IMPORT_CONCURRENCY` (optional) defaults to 10 repos at a time. You can also use `SNYK_IMPORT_CONCURRENCY` or set via CLI flag `--concurrency`. Just 1 repo may have many projects inside which can trigger many files at once to be requested from the user's SCM instance and some may have rate limiting in place. This tool aims to help reduce the risk of hitting a rate limit.
- `SNYK_API` (optional) defaults to `https://api.snyk.io/v1`

**Alternative:** Use `config.toml` to set these values permanently. See [Configuration Guide](../README.md#configuration).

## 3. Download & run

Grab a binary from the [releases page](https://github.com/snyk/snyk-api-import/releases) and run the import command:

```bash
# Auto-detect import targets file from SNYK_LOG_PATH
snyk-api-import import

# Or specify a file explicitly
snyk-api-import import --file=path/to/import-targets.json --source=github

# Adjust concurrency if needed
snyk-api-import import --concurrency=5
```

The tool will look for `<source>-import-targets.json` files in `SNYK_LOG_PATH` or use the file specified with `--file`.

## 4. Review logs
When import is started via Snyk API, many files & targets will be added to an import job. This job when complete will provide logs of what projects could be detected, which failed and any errors that were encountered. For more details see [Import API documentation](https://docs.snyk.io/snyk-api/reference/import-projects-v1)

All logs are located in the `SNYK_LOG_PATH` directory. Logs are in JSON format and can be easily parsed with `jq`.

The `import` command generates several logs:
- `<org-id>.imported-projects.log` - Successfully imported projects
- `<org-id>.failed-imports.log` - Failed import attempts (includes retry details)
- `<org-id>.import-job-results.log` - Detailed import job status and results from the [Import API](https://docs.snyk.io/snyk-api/reference/import-projects-v1)
- `imported-targets.log` - All processed targets

**Note:** The tool automatically retries failed imports up to 3 times, so you may see multiple entries in `failed-imports.log` for the same target.
...
**Note:** Use `--integrationType` instead of the deprecated `--source` flag for this command.
...
```bash
# View all failed imports (includes retry attempts)
jq . logs/<org-id>.failed-imports.log

# Count unique failed targets
jq -s 'unique_by(.target) | length' logs/<org-id>.failed-imports.log

# View detailed import job results
