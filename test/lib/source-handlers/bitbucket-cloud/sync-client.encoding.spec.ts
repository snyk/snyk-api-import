...
describe('BitbucketCloudSyncClient URL encoding', () => {
  ...
  it('encodes branch names with slashes when calling listFiles', async () => {
    ...
    fakeInstance.get.mockResolvedValue({
      ...
      config: {
        url: '/repositories/org/repo/src/feature%2Fbranch/?pagelen=100&max_depth=10',
      },
    });
    mockedAxios.create.mockReturnValue(fakeInstance);

    const client = new BitbucketCloudSyncClient({
      type: 'basic',
      username: 'user',
      appPassword: 'pass',
    } as any);
  ...
  it('encodes workspace and repo for getRepository and listBranches', async () => {
    ...
    fakeInstance.get.mockResolvedValue({
      data: { dummy: true },
      config: { url: '/repositories/org%2Fname/repo%2Fwith%2Fslash' },
    });
    mockedAxios.create.mockReturnValue(fakeInstance);
    ...
    expect(fakeInstance.get.mock.calls[0][0]).toBe(
      `/repositories/${encodeURIComponent('org/name')}/${encodeURIComponent(
        'repo/with/slash',
      )}`,
    );
    expect(fakeInstance.get.mock.calls[1][0]).toBe(
      `/repositories/${encodeURIComponent('org/name')}/${encodeURIComponent(
        'repo/with/slash',
      )}/refs/branches`,
    );
