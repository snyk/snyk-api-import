  ...
  request = (request: { verb: string; url?: string; body?: any } = { verb: 'get' }): Promise<unknown> => {
    return new Promise((resolve) => {
      ...
      if (verb === 'get') {
        ...
        return resolve({
          statusCode: 200,
          data: { name: 'group-test', url: 'https://snyk.example/groups/test', id: 'test-group', orgs: [] },
        });
      }
      if (verb === 'post') {
        // If creating an org (POST /org), return a CreatedOrgResponse-like payload
        if (request.url && request.url.includes('/org')) {
