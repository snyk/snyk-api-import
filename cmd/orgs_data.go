...
func OrgsDataCmd(ctx context.Context, cfg internal.AppConfig) {
	...
	githubOrgs := fs.String("githubOrgs", "", "Comma-separated list of GitHub organization names (for github source)")
	...
	// If githubOrgs flag is empty and source is github, try to get default_org from config
	if *githubOrgs == "" && (*source == "github" || *source == "github-com") {
		if tomlCfg := internal.GetGlobalTOMLConfig(); tomlCfg != nil {
			if integration, err := tomlCfg.GetIntegrationConfig(*source); err == nil && integration.DefaultOrg != "" {
				*githubOrgs = integration.DefaultOrg
				logging.Debugf("Using default GitHub org from config: %s", *githubOrgs)
			}
		}
	}
	...
	if *source == "bitbucket-cloud-app" {
		...
		var orgNames []string
		if *githubOrgs != "" {
			// Explicit org list. This skips the /user/orgs lookup, which returns an
			// empty list for fine-grained personal access tokens.
			for _, part := range strings.Split(*githubOrgs, ",") {
				if trimmed := strings.TrimSpace(part); trimmed != "" {
					orgNames = append(orgNames, trimmed)
				}
			}
			if len(orgNames) == 0 {
				logging.ValidationErrorf("No valid GitHub organization names provided")
				return
			}
			logging.Infof("Processing %d GitHub organization(s) from --githubOrgs", len(orgNames))
		} else {
			...
			if len(orgs) == 0 {
				logging.Warnf("No GitHub organizations found for the authenticated user")
				logging.Warnf("Fine-grained personal access tokens always return an empty list here; specify organizations explicitly via --githubOrgs=\"org1,org2\" or use a classic token with the read:org scope")
			}
		...
		for _, orgName := range orgNames {
			if *skipEmptyOrgs {
				...
				repos, err := internal.FetchGitHubRepos(ctx, auth, orgName, nil)
				if err != nil {
