...
func ImportDataCmd(ctx context.Context, cfg internal.AppConfig) {
	...
	githubRepos := fs.String("githubRepos", "", "Comma-separated list of repository names to limit target generation to (github sources only)")
	...
	// Optional repository allowlist, matching the sync command's --githubRepos.
	...
	if *githubRepos != "" {
		...
		case "github", "github-com", "github-enterprise":
			logging.Infof("Limiting target generation to %d repository/repositories", len(repoFilter))
		default:
	...
	targets, err := internal.GenerateImportTargetsWithLogPath(ctx, orgsDataPath, *source, snykLogPath, repoFilter)
	if err != nil {
