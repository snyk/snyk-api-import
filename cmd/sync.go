...
func SyncCmd(ctx context.Context, cfg internal.AppConfig) {
	...
	githubRepos := fs.String("githubRepos", "", "Comma-separated list of repository names to limit the sync to (github sources only)")
	...
	// Optional repository allowlist. Sync otherwise enumerates every repository
	// the token can see in each org, which for orgs with many public repos is
	// both slow and broader than intended.
	var repoFilter []string
	if *githubRepos != "" {
		for _, part := range strings.Split(*githubRepos, ",") {
			if trimmed := strings.TrimSpace(part); trimmed != "" {
				repoFilter = append(repoFilter, trimmed)
			}
		}
		if len(repoFilter) == 0 {
			logging.ValidationErrorf("--githubRepos was provided but contained no repository names")
			return
		}
		switch *source {
		case "github", "github-com", "github-enterprise", "github-cloud-app":
			logging.Infof("Limiting sync to %d repository/repositories", len(repoFilter))
		default:
			logging.Warnf("--githubRepos is not applicable for source type %s; ignoring", *source)
			repoFilter = nil
		}
	}
	...
	case "github", "github-com", "github-enterprise":
		...
		if err := internal.SyncGitHub(*orgPublicID, *source, "", resolvedTargets, *dryRun, snykLogPath, *enableBranchUpdateFallback, true, repoFilter); err != nil {
			logging.Errorf("Sync failed: %v", err)
		}
